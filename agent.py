...
class NeuralAgent:
    ...
    def act(self, obs: str, score: float, nb_moves:int, nb_episode:int, done: bool, infos: Mapping[str, Any]) -> Optional[str]:
        ...
        if self.transitions:
            ...
            #reward += gg_score
            move_penalty = (nb_moves *0.1)
            #reward = reward - move_penalty
#             print("nb_moves: ", nb_moves, "move_penalty: ", move_penalty, "reward: ", reward)
            if infos["won"]:
                reward += 10
            if infos["lost"]:
                reward -= 10
...
def play(phrase_mode, agent, gg_premodel, path, max_step=50, nb_episodes=20, verbose=True):
    """
...
for phrase_mode in phrase_modes:
    ...
    tr_avg_scores, _ = play(phrase_mode, agent, gg_model, game_path, max_step=step_size, nb_episodes=3000, verbose=False)
    training_scores.append(tr_avg_scores)
    print("Trained in {:.2f} secs".format(time() - start_time))

    print(' =====  Test  ========================================================= ')
    agent.test(method='random')
    test_avg_scores, policy_list = play(phrase_mode, agent, gg_model, game_path, max_step=step_size)  # Medium level game.
...
plt.title("training")
plt.plot(tr_x, training_scores[0], label='Phrase 1')
plt.plot(tr_x, training_scores[1], label='Phrase 2')
plt.plot(tr_x, training_scores[2], label='Phrase 3')
plt.plot(tr_x, training_scores[3], label='random')
plt.plot(tr_x, training_scores[4], label='game_action')
...
plt.title("testing")
plt.plot(testing_scores[0], label='Phrase 1')
plt.plot(testing_scores[1], label='Phrase 2')
plt.plot(testing_scores[2], label='Phrase 3')
plt.plot(testing_scores[3], label='random')
plt.plot(testing_scores[4], label='game_action')
plt.xlabel('no_episode')
plt.ylabel('score')
plt.legend()
plt.show()
