...
def has_cycle(action_cycle, prev_cmd, cur_cmd):
    cycle = False
    if prev_cmd in action_cycle:
        if action_cycle[prev_cmd] == cur_cmd:
            cycle = True
    ...
    for no_episode in range(nb_episodes):
        ...
        while not done:
            ...
            if is_nutral_command(game_command) == False:
                ...
                _gscore = get_command_score(modified_command)
                print("modified_command: ",modified_command, " _gscore: ", _gscore)
            ...
            print("phrase_mode: ",phrase_mode, " no_episode: ",no_episode," steps: ",nb_moves, " command -> ", command, " game_command: ", game_command, " _gscore: ", _gscore, " gg_score: ", gg_score, " score: ", score, " t_score: ", t_score)
            nb_moves += 1
...
for phrase_mode in phrase_modes:
    ...
    tr_avg_scores, _, task_command_no_list, altruistic_command_no_list = play(phrase_mode, agent, gg_model, game_path, max_step=step_size, nb_episodes=nb_episodes, verbose=False)
    ...
    test_avg_scores, policy_list, _, _ = play(phrase_mode, agent, gg_model, game_path, max_step=step_size, nb_episodes=50)  # Medium level game.
...
pickle.dump(training_scores, open( "result/random/training_scores_gg_pos.p", "wb" ) )
pickle.dump(all_task_specific_command_list, open( "result/random/task_gg_pos.p", "wb" ) )
pickle.dump(all_altruistic_command_list, open( "result/random/altruistic_gg_pos.p", "wb" ) )


print("task: ", all_task_specific_command_list)
print("altruism: ", all_altruistic_command_list)
file1 = open('result/random/modified_desc_test_game_action_superhero_10_facts_3000_gg_pos_random.txt', 'w') 
