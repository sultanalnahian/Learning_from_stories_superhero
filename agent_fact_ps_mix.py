...
class NeuralAgent:
    ...
    def act(self, obs: str, score: float, nb_moves:int, nb_episode:int, done: bool, infos: Mapping[str, Any]) -> Optional[str]:
        ...
        input_ = "{}\n{}\n{}\n{}".format(obs, infos["description"], facts, infos["inventory"])
        ...
        if self.transitions:
            with torch.no_grad():
                if is_nutral_command(action) == False:
                    ginput_ids = torch.tensor(tokenizer.encode(infos["description"] +' '+ action, add_special_tokens=True)).unsqueeze(0).cuda() # Batch size 1
            ...
            self.last_g_score += ggscore
            total_score = score + self.last_g_score
            reward = total_score - self.last_score  # Reward is the gain/loss in score.
...
def is_nutral_command(game_command):
    command = game_command
    if game_command in phrase_to_action_pair_map:
        command = phrase_to_action_pair_map[game_command]
    nutral = False
...
def play(phrase_mode, agent, gg_premodel, path, max_step=50, nb_episodes=30, verbose=True):
    ...
    for no_episode in range(nb_episodes):
        ...
        while not done:
            ...
            obs, score, done, infos = env.step(game_command)
            #_gscore = 0
            ...
            print("phrase_mode: ",phrase_mode, " no_episode: ",no_episode," steps: ",nb_moves, " command -> ", command, " game_command: ", game_command, " score: ", score)
            nb_moves += 1
...
pickle.dump(all_nb_moves_list, open( "result/ps_mix/ps_mix_phrase_3_all_nb_moves_list.p", "wb" ) )
pickle.dump(training_scores, open( "result/ps_mix/ps_mix_phrase_3_training_scores.p", "wb" ) )
pickle.dump(testing_scores, open( "result/ps_mix/ps_mix_phrase_3_testing_scores.p", "wb" ) )

pickle.dump(all_task_specific_command_list, open( "result/ps_mix/ps_mix_task_ps_phrase_3.p", "wb" ) )
pickle.dump(all_altruistic_command_list, open( "result/ps_mix/ps_mix_altruistic_ps_phrase_3.p", "wb" ) )


print("task: ", all_task_specific_command_list)
print("altruism: ", all_altruistic_command_list)
file1 = open('result/ps_mix/ps_mix_test_phrases_1_quest_10_facts_4000.txt', 'w') 
