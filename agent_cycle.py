...
class NeuralAgent:
    ...
    def act(self, obs: str, score: float, nb_moves:int, done: bool, infos: Mapping[str, Any]) -> Optional[str]:
...
def play(agent, gg_premodel, path, max_step=50, nb_episodes=10, verbose=True):
    ...
    for no_episode in range(nb_episodes):
        ...
        prev_command = "-"
        while not done:
            command = agent.act(obs, score, nb_moves, done, infos)
            print("command -> ", command)
            obs, score, done, infos = env.step(command)
            if has_cycle(prev_command, command):
                gg_score = 0
            else:
                gg_result = gg_premodel.get_score(command)
                gg_score = gg_result[0][1]
            action_cycle_map[prev_command] = command
            prev_command = command
            score = score + gg_score
#             if gg_result[0][1] < 0:
#                 gg_score = -0.25
#             else:
#                 gg_score = 0.25
            nb_moves += 1
        agent.act(obs, score, nb_moves, done, infos)  # Let the agent know the game is done.
...
game_path = "./tw_games/super_hero__8.ulx"
agent = NeuralAgent()
gg_model = ggModel()
...
play(agent, gg_model, game_path, max_step=step_size, nb_episodes=8000, verbose=False)
